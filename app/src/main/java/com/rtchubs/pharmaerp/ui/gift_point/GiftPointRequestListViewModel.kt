...
class GiftPointRequestListViewModel @Inject constructor(
    private val application: Application,
    private val giftPointRepository: GiftPointRepository
) : BaseViewModel(application) {

    val giftPointsHistoryList: MutableLiveData<List<GiftPointRequest>> by lazy {
        MutableLiveData<List<GiftPointRequest>>()
    }
    ...
    fun getGiftPointsHistory(merchantId: Int) {
        if (checkNetworkStatus()) {
            ...
            viewModelScope.launch(handler) {
                when (val apiResponse = ApiResponse.create(giftPointRepository.getGiftPointRequestList(merchantId))) {
